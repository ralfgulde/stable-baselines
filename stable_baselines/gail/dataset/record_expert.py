...
def generate_expert_traj_HER(model, save_path=None, env=None, n_timesteps=0,
                         n_episodes=100, image_folder='recorded_images'):
    """
    Train expert controller (if needed) and record expert trajectories.
    ...
    assert not record_images, "Images not supportedhere"
    ...
    if isinstance(env.observation_space, spaces.Box) and not record_images:
        ...
        print('the dict obs space will be treated like this: observations = np.array(observations)')
        observations = np.array(observations)
